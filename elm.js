...
var $author$project$Main$deleteElementFromQuestionList = F2(
	function (index, questionlist) {
		return A2(
			$elm$core$List$map,
			...
			A2(
				...
				function (tuple) {
					return !_Utils_eq(index, tuple.a);
				},
...
var $author$project$Main$download = function (sqlContent) {
	return A3($elm$file$File$Download$string, 'Fragen.sql', 'text/plain', sqlContent);
};
var $author$project$Main$getElementFromQuestionList = F2(
	function (index, questionlist) {
		return A2(
			$elm$core$List$map,
			$elm$core$Tuple$second,
...
var $author$project$Main$sqlFromQuestion = F2(
	function (questionType, question) {
		if (questionType.$ === 'ToDelete') {
			return 'DELETE FROM questions WHERE questiontext= \'' + (question + '\'; \n');
		} else {
			return 'INSERT INTO questions SET questiontext=\'' + (question + '\'; \n');
		}
	});
var $author$project$Main$sqlFromList = F2(
	function (questionlist, questionType) {
		return A3(
			$elm$core$List$foldr,
			...
			A2(
				$elm$core$List$map,
				function (question) {
					return A2($author$project$Main$sqlFromQuestion, questionType, question);
				},
				questionlist));
	});
			...
			case 'Delete':
				...
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							deletedQuestions: _Utils_ap(
								model.deletedQuestions,
								A2($author$project$Main$getElementFromQuestionList, index, model.questions)),
							questions: A2($author$project$Main$deleteElementFromQuestionList, index, model.questions)
						}),
					$elm$core$Platform$Cmd$none);
			case 'Download':
				return _Utils_Tuple2(
